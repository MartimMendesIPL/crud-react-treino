...
export const create = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const client = await clientService.create({ name, email, phone, address, vat_number, notes });
...
export const update = async (req: Request, res: Response): Promise<void> => {
  try {
    const { name, email, phone, address, vat_number, notes } = req.body;
    if (!name) { res.status(400).json({ error: 'Field "name" is required' }); return; }
    const client = await clientService.update(req.params.id, { name, email, phone, address, vat_number, notes });
    if (!client) { res.status(404).json({ error: 'Client not found' }); return; }
    res.json(client);
  } catch (err) {

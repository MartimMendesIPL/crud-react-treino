...
export const create = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!name || unit_price == null || !unit) {
      res.status(400).json({ error: 'Fields name, unit_price, and unit are required' });
      return;
    }
    const product = await productService.create({ name, description, unit_price, unit });
...
export const update = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const product = await productService.update(req.params.id, { name, description, unit_price, unit });
    if (!product) { res.status(404).json({ error: 'Product not found' }); return; }
    res.json(product);
  } catch (err) {

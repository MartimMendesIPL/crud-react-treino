...
export default function CrudPage<T extends { id: number | string }>({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      load();
      sileo.success({
        title: t("admin.crud.successTitle"),
        description: editing
          ? t("admin.crud.updateDescription")
          : t("admin.crud.createDescription"),
      });
    } catch (e: unknown) {
      sileo.error({
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      ...
      // Ignore clicks on any react-select sub-element (menu portal teleported to
      // <body>, clear "×" button, dropdown indicator, control, value, etc.).
      // We use a broad attribute selector so we never have to enumerate every
      // individual sub-class (e.g. __clear-indicator, __control, __menu …).
      if ((target as Element).closest?.("[class*='react-select__']")) {
        return;
      }
      if (
        panelOpen &&
          ...
          >
            ...
            <div className="crud-panel-header">
              <h2 className="crud-panel-title">
                {editing
                  ? t("admin.crud.editTitle", { resource: title })
                  : t("admin.crud.newTitle", { resource: title })}
              </h2>
                ...
                <div className="crud-field" key={f.name}>
                  ...
                  {f.type === "async-select" ? (
                    ...
                    <ReactSelect
                      ...
                      value={
                        f.options?.find(
                          (o) => o.value === String(formData[f.name] ?? ""),
                        ) ?? null
                      }
                      onChange={(opt: SingleValue<SelectOption>) =>
                        setFormData({

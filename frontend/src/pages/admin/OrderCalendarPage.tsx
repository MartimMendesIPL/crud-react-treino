      ...
      <div className="cal-header">
        ...
        <div className="cal-filters">
          {ORDER_STATUSES.map((s) => (
            ...
            >
              {s === "all" ? t("common.all", "All") : t(`status.${s}`, s)}
            </button>
          ))}
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="cal-week-items">
                ...
                {items.map((o) => {
                  const secColor = getSectionColor(o.section_id);
                  ...
                  return (
                    <button
                      ...
                      style={{
                        backgroundColor: hexToRgba(secColor, 0.15),
                      }}
                      onClick={(e) => {
                        e.stopPropagation();
                        openPanel(o);
                    ...
                    >
                      ...
                      {itemsSummary ? (
                        <div className="cal-card-products">
                          {itemsSummary.map((p, idx) => (
                            <div key={idx} className="cal-card-product-line">
                              <span className="cal-card-qty">{p.qty}x</span>{" "}
                              <span className="cal-card-pcode">{p.code}</span>
                              {" - "}
                              <span className="cal-card-pname">{p.name}</span>
                            </div>
                          ))}
                          {o.items && o.items.length > 3 && (
                            <div className="cal-card-more">
                              +{o.items.length - 3}{" "}
                              {t("admin.calendar.more", "more")}
                            </div>
                          )}
                        </div>
                      ) : (
                        <div className="cal-card-products cal-card-no-items">
                          {t("admin.calendar.noItems", "No items")}
                        </div>
                      )}
            ...
            <div className="cal-panel-header">
              <div className="cal-panel-header-left">
                <div
                  ...
                  style={{
                    backgroundColor: getSectionColor(panelOrder.section_id),
                  }}
                />
                <div>
                  <h2 className="cal-panel-title">{panelOrder.reference}</h2>
                  <span className="cal-panel-subtitle">
                    {clientName(panelOrder.client_id)}
                  </span>
                </div>
              </div>
              <button className="cal-panel-close" onClick={closePanel}>
                <X size={18} />
              </button>
            </div>
              ...
              <div className="cal-panel-body">
                ...
                <div className="cal-panel-section">
                  <h3 className="cal-panel-section-title">
                    ...
                    {t("admin.calendar.generalInfo", "General Information")}
                  </h3>
                  ...
                  <div className="cal-panel-info-grid">
                    <div className="cal-panel-info-item">
                      ...
                      <div>
                        <div className="cal-panel-info-label">
                          {t("admin.orders.reference", "Reference")}
                        </div>
                    ...
                    <div className="cal-panel-info-item">
                      ...
                      <div>
                        <div className="cal-panel-info-label">
                          {t("admin.orders.section", "Section")}
                        </div>
                        <div className="cal-panel-info-value">
                          <span
                          ...
                          {panelOrder.section_name ??
                            sectionName(panelOrder.section_id)}
                        </div>
                      </div>
                    </div>
                          ...
                          <span
                            ...
                            style={{
                              backgroundColor: hexToRgba(
                                STATUS_COLOR[panelOrder.status] ?? "#6b7280",
                                0.12,
                              ),
                              color:
                                STATUS_COLOR[panelOrder.status] ?? "#6b7280",
                    ...
                    <div className="cal-panel-info-item">
                      ...
                      <div>
                        <div className="cal-panel-info-label">
                          {t("admin.orders.dueDate", "Due Date")}
                        </div>
                        <div className="cal-panel-info-value">
                          {formatFullDate(panelOrder.due_date)}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="cal-panel-info-item">
                      ...
                      <div>
                        <div className="cal-panel-info-label">
                          {t("admin.orders.created", "Created")}
                        </div>
                        <div className="cal-panel-info-value">
                          {formatFullDate(panelOrder.created_at)}
                        </div>
                      </div>
                    </div>
                    ...
                    {panelOrder.proposal_reference && (
                      <div className="cal-panel-info-item">
                        ...
                        <div>
                          <div className="cal-panel-info-label">
                            {t("admin.orders.proposal", "Proposal")}
                          </div>
                          <div className="cal-panel-info-value">
                            {panelOrder.proposal_reference}
                          </div>
                    ...
                    {clientMap.get(panelOrder.client_id)?.address && (
                      <div className="cal-panel-info-item cal-panel-info-full">
                        ...
                        <div>
                          <div className="cal-panel-info-label">
                            {t("admin.calendar.address", "Address")}
                          </div>
                          <div className="cal-panel-info-value">
                            {clientMap.get(panelOrder.client_id)?.address}
                          </div>
                ...
                <div className="cal-panel-section">
                  <h3 className="cal-panel-section-title">
                    <Package size={16} />
                    {t("admin.calendar.orderLines", "Order Lines")}
                    {panelOrder.items && (
                  ...
                  {panelOrder.items && panelOrder.items.length > 0 ? (
                    <div className="cal-panel-items">
                      {panelOrder.items.map((item, idx) => {
                        const prod = productMap.get(item.product_id);
                        return (
                          <div key={item.id} className="cal-panel-item-card">
                            ...
                            <div className="cal-panel-item-details">
                              ...
                              <div className="cal-panel-item-detail">
                                <span className="cal-panel-item-detail-label">
                                  {t("admin.orders.unitPrice", "Unit Price")}
                                </span>
                                <span className="cal-panel-item-detail-value">
                    ...
                    <div className="cal-panel-empty">
                      {t("admin.orders.noItems", "No items yet.")}
                    </div>
                  )}
                </div>
              </div>

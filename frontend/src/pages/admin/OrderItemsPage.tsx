...
export default function OrderItemsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get<OrderItem[]>(`/order-items?order_id=${id}`)
      .then((i) => {
        if (!ignore) setItems(i);
      })
      .catch(() => {})
      .finally(() => {
  ...
  const openEdit = async (item: OrderItem) => {
    setEditing(item);
    setFormData({
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/order-items/${editing.id}`, formData);
      } else {
        await api.post(`/order-items?order_id=${id}`, formData);
      }
      setModalOpen(false);
      load();
    } catch (e: unknown) {
    ...
    <div className="crud-page">
      <div className="crud-header">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <h1 className="crud-title">{t("admin.orders.itemsTitle", { id })}</h1>
        </div>
      ...
      <div className="crud-table-wrap">
        {loading ? (
          ...
          <div className="crud-empty">{t("admin.orders.noItems")}</div>
        ) : (
        ...
        <div className="crud-modal-overlay" onClick={() => setModalOpen(false)}>
          <div className="crud-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="crud-modal-body">
              ...
              {/* Notes */}
              <div className="crud-field">
                <label className="crud-label">{t("admin.orders.notes")}</label>
                <textarea

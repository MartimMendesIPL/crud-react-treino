...
export default function ProposalCalendarPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([p, c]) => { if (!ignore) { setProposals(p); setClients(c); } })
      .catch(() => {})
  ...
  if (loading) {
    return <div className="crud-loading"><Loader2 size={24} className="crud-spin" /> Loading calendar…</div>;
  }
      ...
      <div className="cal-header">
        <div className="cal-header-left">
          ...
          <div className="cal-nav">
            <button className="cal-nav-btn" onClick={prevWeek}><ChevronLeft size={18} /></button>
            <span className="cal-week-label">{rangeLabel}</span>
            <button className="cal-nav-btn" onClick={nextWeek}><ChevronRight size={18} /></button>
        ...
        <div className="cal-filters">
          {["all", "draft", "pending", "approved", "rejected"].map((s) => (
            <button
              ...
              onClick={() => setFilter(s)}
              style={filter === s && s !== "all" ? { borderColor: STATUS_COLOR[s], color: STATUS_COLOR[s] } : undefined}
            >
              ...
              <div className="cal-week-items">
                ...
                {items.map((p) => (
                  <button
                    ...
                    style={{ borderLeftColor: STATUS_COLOR[p.status] ?? "#6366f1" }}
                    onClick={() => navigate(`/admin/proposals/${p.id}/items`)}
                    title={`${p.reference} — ${clientName(p.client_id)} (${p.status})`}
                  >
                    <span className="cal-week-item-ref">{p.reference}</span>
                    <span className="cal-week-item-client">{clientName(p.client_id)}</span>
                    <span
